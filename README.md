...
2. Проверьте команду в консоли:
   ```
   net user sshuser
    ```
   найдите: Пароль действителен до
4. если обнаружили сегодняшнюю дату, откройте cmd от имени администратора и впишите следующую команду, поменяв пароль на новый свой(это поменяет на ваш новый пароль и установит бессрочную дату действия пароля для sshuser):

```
net user sshuser "NewPassword123!" /passwordchg:no /expires:never && net start sshd
```

6. Также сделайте действие пароля и учетной записи бессрочными:
